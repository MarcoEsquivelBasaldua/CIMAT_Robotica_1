...
while running:
    ...
    if state > 200:
        ...
        pygame.draw.circle(screen, purple, x_init, 8)     # Draw x_init and x_goal point again
        pygame.draw.circle(screen, red, mouse_pos, 20)
        pygame.display.update()

        # Plot trayectorie fron x_init to x_goal
        if goal_reached:
            # Find closest node with cheapest cost
            d_max = 30.0
            last_cost = float('inf')

            for k in range(0, len(T)):
                if dist(x_goal, T[k][0]) < d_max and T[k][1] < last_cost:
                    index = k
                    d_max = dist(x_goal, T[k][0])
                    last_cost = T[k][1]


            print('Goal Reached')
            print('Cost to the goal ', T[index][1])
            print('Nodes expantion ', nodes+1)

...
int main(void){
    ...
    // Vertices on visual polygon generated
...
void visual(pair<double,double> robot, vector<pair<double,double> > mapCoordinates, vector<pair<double,pair<double,double> > > mapCoordinates_byAngle, vector<pair<double,double> >& polygon){
    ...
    for(int i=0; i<mapCoordinates_byAngle.size(); i++){
        ...
        last_distance = dist(p_polygon, robot);
        new_distance = dist(p2, robot);
        double theta = atan2(p2.second - robot.second, p2.first - robot.first);

        if(new_distance < last_distance and (theta < -Pi/2.0 or theta > Pi/2.0) and add == 1)
            polygon.push_back(p2);


        polygon.push_back(p_polygon);

        if(new_distance < last_distance and (theta >= -Pi/2.0 and theta <= Pi/2.0) and add == 1)
            polygon.push_back(p2);
